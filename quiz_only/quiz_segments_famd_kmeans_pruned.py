...
def famd_fit_scores(df_catnum: pd.DataFrame, explained_target: float, n_max: int, seed: int):
    """Fit FAMD with many comps, choose n_keep by cumulative variance, return famd model, X (scores), n_keep, expl array."""
...
def cluster_and_profile(tag: str, df_base: pd.DataFrame, famd_vars_cat: List[str], famd_vars_num: List[str],
    ...
    np.save(os.path.join(OUTDIR, f"{tag}_famd1_explained.npy"), expl1)
    # prune
    ...
    plot_sil_curve(sil_curve, os.path.join(OUTDIR, f"{tag}_silhouette.png"), f"{tag}: Silhouette by k (FAMD pruned)")
...
# Small text summary for your report
